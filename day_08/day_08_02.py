...
def get_circuits(distances):
    ...
    for distance, i, j in distances:
        ...
        # after each iteration check if all boxes are connected in one circuit
        if len(circuits) == 1 and len(circuits[0]) == num_boxes:
            print("All boxes connected in one circuit!")
            print("X coordinates of the last connection added are:", junction_boxes[i].x, junction_boxes[j].x)
            print("Result:", junction_boxes[i].x * junction_boxes[j].x)
            return
    return

get_circuits(shortest_distances)
